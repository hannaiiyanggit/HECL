...
from dataset.bar import BAR, get_action_dataset_train_test
...
class BaseTrainer:
    ...
    def __call__(self):
        ...
        for _ in range(self.cur_epoch, args.epoch + 1):
            ...
            '''if "cond_test_accuracy" not in self.cond_on_best_val_log_dict:
                print(self.cond_on_best_val_log_dict)
            cond_test_acc = self.cond_on_best_val_log_dict["cond_test_accuracy"] * 100

            print(
                f"[{self.cur_epoch}/{args.epoch}] "
                f"ID Acc: {cond_test_acc:.2f} "
                f""
            )'''
            self.cur_epoch += 1
    ...
    def eval(self):
        val_log_dict = self._eval_split_bar(self.val_loader, "val")
        test_log_dict = self._eval_split_bar(self.test_loader, "test")

        early_stop_metric_result = val_log_dict['val_accuracy']
        ...
        print(
            ...
            f"[{self.cur_epoch}/{self.args.epoch}] "
            f"Val Acc: {val_log_dict['val_accuracy']*100:.2f} "
            f"Test Acc: {test_log_dict['test_accuracy']*100:.2f} "
            f"Best Val Acc: {self.cond_best_acc*100:.2f} "
            f"Cond Test Acc: {self.cond_on_best_val_log_dict['cond_test_accuracy']*100:.2f} "
            f""
        )
    ...
    @torch.no_grad()
    def _eval_split_bar(self, loader, split, dset_name="bar"):
        ...
        for data_dict in pbar:
            ...
            '''feature_ref = self.classifier(img)
        ...
        log_dict = {f'{split}_accuracy': acc}
        ...
        log_dict.update(
            {
                ...
                f"{split}_avg_gap": (both_gap+bg_gap+co_occur_obj_gap)/3,
            }
        )
