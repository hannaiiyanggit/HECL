...
class HeclTrainer(BaseTrainer):
    ...
    def train(self):
        ...
        self._set_train()
        losses = AverageMeter("Loss", ":.4e")
        easy_losses = AverageMeter("classifier_Loss", ":.4e")
        hard_losses = AverageMeter("contrastive_loss", ":.4e")
        ...
        for data_dict in pbar:
            ...
            with torch.cuda.amp.autocast(enabled=args.amp):
                ...
                pos_rank = torch.as_tensor(pos_rank).detach()[:, np.newaxis]
                image_rank = torch.as_tensor(image_rank).detach()[:, np.newaxis]

                # common loss
                classifier_loss1 = F.cross_entropy(output[:int(image_.shape[0] / 2)],
                                            obj_gt)
                # pos loss
                classifier_loss2 = F.cross_entropy(output[int(image_.shape[0] / 2):],
                                            obj_gt)* (1 + pos_rank / self.max_rank)

                loss = classifier_loss2.sum() + self.cur_epoch/self.args.epoch*self.args.classifier_weight * classifier_loss1.sum()
                easy_loss = classifier_loss1.sum()
                hard_loss = classifier_loss2.sum()
            ...
            easy_losses.update(easy_loss.item(), int(image.size(0) / 2))
            hard_losses.update(hard_loss.item(), int(image.size(0) / 2))

            pbar.set_description(
                f"[{self.cur_epoch}/{args.epoch}] loss: {losses.avg:.4f} "
                f"easy_loss: {easy_losses.avg:.4f} "
                f"hard_loss: {hard_losses.avg:.4f}")
        if self.args.lr_decay is not None:
            self.scheduler.step()
